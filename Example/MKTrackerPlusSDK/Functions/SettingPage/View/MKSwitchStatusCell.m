...
- (void)switchViewValueChanged{
    if ([self.delegate respondsToSelector:@selector(needChangedCellSwitchStatus:indexPath:)]) {
        [self.delegate needChangedCellSwitchStatus:self.switchView.isOn indexPath:self.indexPath];
    }
}
