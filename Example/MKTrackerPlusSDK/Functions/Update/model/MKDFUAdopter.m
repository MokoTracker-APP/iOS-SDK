...
- (void)dfuUpdateWithFileName:(NSString *)fileName target:(UIViewController *)controller{
    ...
    [self.dfuModel dfuUpdateWithFileUrl:url progressBlock:^(CGFloat progress) {
        ...
        [[MKHudManager share] showHUDWithTitle:@"Update firmware successfully!" inView:controller.view isPenetration:NO];
...
- (void)updateComplete:(UIViewController *)vc{
    ...
    [MKTrackerCentralManager sharedDealloc];
    [vc.navigationController popToRootViewControllerAnimated:YES];
    [self performSelector:@selector(postDeallocNotification) afterDelay:1.f];
}
