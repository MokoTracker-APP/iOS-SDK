...
@interface MKTrackingIntervalCell ()
...
@implementation MKTrackingIntervalCell

+ (MKTrackingIntervalCell *)initCellWithTableView:(UITableView *)tableView {
    MKTrackingIntervalCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKTrackingIntervalCellIdenty"];
    if (!cell) {
        cell = [[MKTrackingIntervalCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKTrackingIntervalCellIdenty"];
    }
    return cell;
}
...
- (UILabel *)msgLabel {
    if (!_msgLabel) {
        ...
        _msgLabel.attributedText = [MKAttributedString getAttributedString:@[@"Tracking Interval",@"   (0min ~ 100min)"] fonts:@[MKFont(15.f),MKFont(12.f)] colors:@[DEFAULT_TEXT_COLOR,RGBCOLOR(223, 223, 223)]];
    }
    return _msgLabel;
}
